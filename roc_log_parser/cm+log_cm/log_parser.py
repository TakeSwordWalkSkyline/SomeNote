...
rst_r = r'\[reset\]\s+(\d+)'
strt_r = r'\[start\]\s+(\d+)'
chk_r = r'\[check\]\s+(\d+)\s+(\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)'
...
def count_if_larger(arr, value):
    count = 0
    for v in arr:
        if (v[0] > value) and (v[1] > value): #v[0] is cm, v[1] is log_cm
            count += 1
    return count
    ...
    with open(log, 'r') as log_f:
        for line in log_f:
            ...
            if type == 'LOG':
                ...
                start = int(m.group(2))
                cm = float(m.group(3))
                log_cm = float(m.group(4))
                if case['type'] == JN:
                    if candidate is None:
                        candidate = {m.group(1): {start: [cm, log_cm]}}
                    elif m.group(1) not in candidate:
                        candidate[m.group(1)] = {start: [cm, log_cm]}
                    else:
                        for k, v in candidate[m.group(1)].iteritems():
                            if v[0] < cm: 
                                candidate[m.group(1)][k][0] = cm
                            if v[1] < log_cm:
                                candidate[m.group(1)][k][1] = log_cm
                elif case['type'] == JW:
                    if candidate is None:
                         candidate = {m.group(1): {start: [cm, log_cm]}}
                    elif m.group(1) not in candidate:
                        ...
                        candidate[m.group(1)][start] = [cm, log_cm]
                    elif candidate[m.group(1)][start][0] < cm:
                        candidate[m.group(1)][start][0] = cm
                        candidate[m.group(1)][start][1] = log_cm
                    elif candidate[m.group(1)][start][1] < log_cm:
                        candidate[m.group(1)][start][0] = log
                        candidate[m.group(1)][start][1] = log_cm
                    else:
                        candidate[m.group(1)][start][0] = candidate[m.group(1)][start][0]
                        candidate[m.group(1)][start][1] = candidate[m.group(1)][start][1]
                else:
